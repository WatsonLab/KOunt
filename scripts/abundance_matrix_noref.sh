...
for file in $Path*noref_KOunt; do cat $file <(awk '{print $1}' $file | cat - $KEGGS | sort | uniq -u | sed 's/$/ 0/') >> "$file"_tmp && mv "$file"_tmp "$file"; done #adds zeros for samples not containing any hits a of particular kegg
for file in $Path*noref_KOunt; do sed -i '1 i\KEGG '${file##*/}'' $file && sed -i 's/_noref_KOunt//' $file; done #adds title line
awk 'BEGIN { OFS = ","} {samples[$1] = samples[$1] OFS $2} END {print "KEGG" samples["KEGG"]; delete samples["KEGG"]; for (KEGG in samples) print KEGG samples[KEGG]}' $Path*_noref_KOunt | sed 's/_noref_KOunt//g' > "$Path"hits #merges all of the samples hits
